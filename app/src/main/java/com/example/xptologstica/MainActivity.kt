package com.example.xptologstica
...
/*  PARA NAO DEIXAR O CAMPO VAZIO
            if (TextUtils.isEmpty(binding.salarioFixo.getText)) {
                binding.salarioFixo.setError("Salario não pode ser vazio!")
            } else if (TextUtils.isEmpty(binding.qtdKm.getText)){
                binding.salarioFixo.setError("KM não pode ser vazio!")
            }
*/
//ola
            ...
            val result: Double = nsalariofixo * (cmss / 100)*nkmperc

            val total: Double = result + nsalariofixo



           // comissao.text = "Sua comissão é: R$ %.2f".format(comissao)
           // resultado.text = "Salário final é: R$ %.2f".format(total)

            Toast.makeText(this, "Sua Comissão é = $result ", Toast.LENGTH_SHORT).show()

            Toast.makeText(this, "O Total Do Seu Pagamento É = $total ", Toast.LENGTH_SHORT).show()
        }
